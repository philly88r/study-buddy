...
# Exempt the login route from CSRF protection for the GET request
@csrf.exempt
...
def list_blogs():
    try:
        print("Fetching blog posts from database")
        ...
        posts_data = [{
            ...
            'title': post.title,
            'slug': post.slug,
            'introduction': post.introduction[:200] + '...' if len(post.introduction) > 200 else post.introduction
        } for post in blog_posts]
        
        return jsonify({'blog_posts': posts_data})
    except Exception as e:
        error_msg = str(e)
        print(f"Error in list_blogs: {error_msg}")
...
def generate_blog():
    try:
        print("Received blog generation request")
        data = request.get_json()
        print("Request data:", data)
        ...
        print(f"Starting blog generation for keywords: {keywords}")
        blog_posts = blog_generator.bulk_generate(keywords)
        print(f"Successfully generated {len(blog_posts)} blog posts")
        
        saved_posts = []
        for post in blog_posts:
            try:
                print(f"Saving blog post: {post['title']}")
                blog_post = BlogPost(
                    title=post['title'],
                    slug=post['slug'],
                ...
                db.session.commit()  # Commit each post individually
                print(f"Successfully saved blog post: {post['title']}")
                ...
                db.session.rollback()
                return jsonify({'error': f'Error saving blog post: {str(e)}'}), 500
        
        print(f"Successfully saved {len(saved_posts)} blog posts")
        return jsonify({'blog_posts': saved_posts})
        
    except Exception as e:
        error_msg = str(e)
        print(f"Error in generate_blog: {error_msg}")
        return jsonify({'error': error_msg}), 500
    ...
    try:
        ...
        return jsonify({
            ...
            'blog': {
                ...
                'title': blog_post.title,
                'slug': blog_post.slug,
                'url': url_for('view_blog', slug=blog_post.slug, _external=True)
            }
        }), 201
...
@app.route('/create_admin', methods=['GET', 'POST'])  # Support both URL formats
def create_admin():
    try:
        ...
        if request.method == 'POST':
            ...
            # Debug logging
            print(f"Received create admin request for username: {username}")
            ...
            try:
                ...
                new_admin = User(
                    ...
                    is_admin=True,
                    email=f"{username}@admin.com"  # Add a default email
                )
                ...
                flash('Error creating admin user. Please check database connection.', 'error')
                return render_template('create_admin.html')

        return render_template('create_admin.html')
    except Exception as e:
        print(f"Error in create_admin route: {str(e)}")
        import traceback
        traceback.print_exc()  # Print full stack trace
        flash('An error occurred. Please check database connection.', 'error')
        return render_template('create_admin.html')
